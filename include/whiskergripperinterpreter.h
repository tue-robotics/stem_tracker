...
private:
    ...
    bool m_has_nominal_values, m_grasp_whisker_touched, m_prev_sample_whisker_touched, m_pressure_sensor_is_touched;
    ...
    float m_pressure_sensor_touched_normalized_threshold;
    std::vector<float> m_whisker_touched_max, m_pressure_sensor_touched_max;
...
public:
    ...
    WhiskerGripperInterpreter(RobotStatus* p_robot_status, StemTrackController* p_stemtrack_controller) : m_p_robot_status(p_robot_status),
        ...
        m_grasp_whisker_touched(false), m_prev_sample_whisker_touched(false), m_pressure_sensor_is_touched(false) {}
    ...
    inline void setNumberOfPressureSensors(const int& n) { m_n_pressure_sensors = n; m_nominal_pressure_sensor_values.assign(n,0.0);
                                                           m_pressure_sensor_touched_max.assign(n,0.0); }
    ...
    inline void setPressureSensorTouchedMax(const std::vector<float> max) { m_pressure_sensor_touched_max = max; }
    inline void setPressureSensorTouchedNormalizedThreshold(const float threshold) { m_pressure_sensor_touched_normalized_threshold = threshold; }
    ...
    inline void setWhiskerTouchedNormalizedThreshold(const float threshold) { m_normalized_whisker_touched_threshold = threshold; }
